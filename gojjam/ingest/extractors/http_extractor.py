...
class HTTPExtractor(BaseExtractor):
    ...
    def extract(self,current_page:Optional[Any]= None):
  
        source_cfg = self.model_info["source_config"]
        auth_obj = self._get_auth_strategy(source_cfg)
        if getattr(self.model_info["source_config"],"cursor"):
            cursor = getattr(self.model_info["source_config"],"cursor")
            if cursor.value_location.type == "QUERY":
               
                base_endpoint = str(source_cfg.endpoint).rstrip("/")
                formatted_url = cursor.value_location.location.format(
                ...
                with requests.Session() as session:
                    ...
                    response = session.get(str(formatted_url))
                    response.raise_for_status()
                    full_data = response.json()
                    batch_df = pd.DataFrame(full_data)
